...
describe("a number question", () => {
  ...
  it("names the lower bound when that is the one that broke", async () => {
    render(
      <NumberQuestion id="wp" label="Wattpiek" value={null} min={500} max={30000} unit="Wp" onChange={() => {}} />,
    );
  ...
  it("reports a value inside the bounds and shows nothing wrong", async () => {
    ...
    expect(screen.getByLabelText(/wattpiek/i)).toHaveAttribute("aria-invalid", "false");
  ...
  it("takes a value the parent changed from somewhere else", () => {
    ...
    rerender(
      <NumberQuestion id="wp" label="Wattpiek" value={4200} min={1} max={30000} unit="Wp" onChange={() => {}} />,
    );
  ...
  it("is labelled, so the field can be reached by its name", () => {
    render(
      <NumberQuestion id="wp" label="Wattpiek" value={7000} min={1} max={30000} unit="Wp" onChange={() => {}} />,
    );
