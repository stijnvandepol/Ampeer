...
describe("a link with a token in its fragment", () => {
  ...
  it("opens the new-password form on a reset fragment when nobody is signed in", async () => {
    ...
    // a navigation: the page, not the browser, took the token out of
    // history. The first argument is `null`, not `expect.anything()` (which
    // Vitest's matcher explicitly refuses to match against `null`): the
    // setup above put the browser in a state of `null` before this render,
    // and fragment.ts's own contract is to carry that state through
    // unchanged rather than clobber it, which this pins to the concrete
    // value rather than to "some value or other".
  ...
  it("confirms an address only after the first me/, and asks me/ again when signed in", async () => {
    ...
    render(<AccountPage />);
    expect(
      await screen.findByText("Uw e-mailadres is bevestigd."),
    ).toBeInTheDocument();
